...
pub fn from_reader(mut reader: impl Read) -> MyResult<CountryBoundaries> {
    ...
    for _ in 0..geometry_sizes_count {
        geometry_sizes.insert(read_string(&mut reader)?, read_f64(&mut reader)?);
    }
...
fn read_areas(reader: &mut impl Read) -> MyResult<(String, Multipolygon)> {
    Ok((
        read_string(reader)?,
        Multipolygon {
            outer: read_polygons(reader)?,
            inner: read_polygons(reader)?,
        },
    ))
}
...
mod tests {
    ...
    fn test_read_ring() {
        ...
        for i in 0..empty.len() - 1 {
            assert!(read_ring(&mut &empty[0..i]).is_err());
        }
