...
pub enum Error {
    ...
    MinLatitudeGreaterThanMaxLatitude { min_latitude: f64, max_latitude: f64 }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Error::LatitudeOutOfBounds {param, latitude } => {
                write!(f, "{param} {latitude} is out of bounds, must be within -90.0 and +90.0")
            }
            Error::MinLatitudeGreaterThanMaxLatitude {min_latitude, max_latitude } => {
